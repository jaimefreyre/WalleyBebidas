...
class ProductData {
	...
	public function add(){
		...
		$sql .= "values (\"$this->barcode\",\"$this->name\",\"$this->description\",\"$this->price_in\",\"$this->price_out\",\"$this->user_id\",\"$this->presentation\",\"$this->unit\",$this->category_id,$this->inventary_min, \"$this->created_at\" )";
		print_r($sql);
...
// partiendo de que ya tenemos creado un objecto ProductData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set barcode=\"$this->barcode\",name=\"$this->name\",price_in=\"$this->price_in\",price_out=\"$this->price_out\",user_id=\"$this->user_id\",unit=\"$this->unit\",presentation=\"$this->presentation\",category_id=$this->category_id,inventary_min=\"$this->inventary_min\",description=\"$this->description\",is_active=\"$this->is_active\" where id=$this->id";
		// print_r($sql);
		Executor::doit($sql);
	}
	...
	public static function disponibleXsucursal($userId){
		$sql = "select * from ".self::$tablename." where user_id=$userId ";
		$query = Executor::doit($sql);
		return Model::many($query[0],new ProductData());
	}

	public static function disponibleXsucursalXdetalle($p){
