...
// partiendo de que ya tenemos creado un objecto OperationData previamente utilizamos el contexto
 ...
 $sql = "select * from ".self::$tablename." where date(created_at) >= \"$start\" and date(created_at) <= \"$end\" and product_id=$product order by created_at desc";
	...
	public static function productoXusuario($userId, $start,$end){
 $sql = "select * from ".self::$tablename." where date(created_at) >= \"$start\" and date(created_at) <= \"$end\" and user_id=$userId order by created_at desc";
		if($start == $end){
		 $sql = "select * from ".self::$tablename." where date(created_at) = \"$start\" order by created_at desc";
		}
		$query = Executor::doit($sql);
	...
	public static function contadorXsucursal($product_id, $user_id){
		$q=0;
		$operations = self::traerproductosxsucursal($product_id, $user_id);
		$input_id = OperationTypeData::getByName("entrada")->id;
		$output_id = OperationTypeData::getByName("salida")->id;
		foreach($operations as $operation){
				if($operation->operation_type_id==$input_id){ $q+=$operation->q; }
	...
	public static function traerproductosxsucursal($product_id, $user_id){
		$sql = "select * from  ".self::$tablename." OP INNER JOIN  ".self::$ventas." voi where OP.sell_id = voi.id and OP.product_id=$product_id and voi.user_id=$user_id order by OP.created_at desc";
		// print_r($sql);
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}
